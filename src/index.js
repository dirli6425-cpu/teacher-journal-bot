    ...
    try {
        ...
        if (path==="/api/meals" && request.method==="POST") {
            ...
            await env.DB.prepare(`INSERT INTO meals(student_id,benefit,orphan,note,added_at,updated_at) VALUES(?,?,?,?,?,?)
                ON CONFLICT(student_id) DO UPDATE SET benefit=excluded.benefit,orphan=excluded.orphan,note=excluded.note,updated_at=excluded.updated_at`)
                .bind(studentId,benefit,orphan,note,now,now).run();
