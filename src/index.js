...
async function handleWebApi(request, env, url) {
    try {
        ...
        if (path==="/api/meals" && request.method==="GET") {
            ...
            const rows=(await env.DB.prepare(`SELECT m.student_id,s.name,m.benefit,m.note,m.added_at,m.updated_at
        ...
        if (path==="/api/meals" && request.method==="POST") {
            ...
            const studentId=Number(body.student_id),benefit=body.benefit?1:0,note=String(body.note||"").trim().slice(0,300);
            ...
            await env.DB.prepare(`INSERT INTO meals(student_id,benefit,note,added_at,updated_at) VALUES(?,?,?,?,?)
                ON CONFLICT(student_id) DO UPDATE SET benefit=excluded.benefit,note=excluded.note,updated_at=excluded.updated_at`)
                .bind(studentId,benefit,note,now,now).run();
            await webAudit(env,actor,"meal_save",st.name+" / "+(benefit?"льгота":"без льготы"));
